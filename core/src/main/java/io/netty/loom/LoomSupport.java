...
public final class LoomSupport {
	...
	static {
		...
		try {
			...
			scheduler = findVarHandle(Class.forName("java.lang.VirtualThread"), "scheduler",
					Thread.VirtualThreadScheduler.class);
		} catch (Throwable e) {
			carrierThread = null;
			scheduler = null;
			error = e;
	...
	private static VarHandle findVarHandle(Class<?> declaringClass, String fieldName, Class<?> fieldType) {
		try {
			...
			throw new RuntimeException("Cannot get VarHandle for " + declaringClass.getSimpleName() + "." + fieldName,
					e);
		}
	}
